...
CFGNode* CFGNode::CreateClone(Graph* graph) {
...
std::string CFGNode::GetInstruction() {
	...
	msg << id_ << ":" << title_;
	switch (opcode()) {
	case CFGNode::UOpcode::phi:
		msg << "[" << std::hex << (inputs()[0] >> VARIABLE_ID_SHIFT) << "]";
		break;
	case CFGNode::UOpcode::str:
	case CFGNode::UOpcode::lda:
	case CFGNode::UOpcode::lod:
		msg << "[" << std::hex << (inputs()[0] >> VARIABLE_ID_SHIFT) << "]";
		break;
	}
	return msg.str();
